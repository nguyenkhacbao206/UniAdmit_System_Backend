...
class AdmissionService {
  async runAdmission(roundId) {
    ...
    await round.save();
    try {
      await _admissionResult.default.deleteMany({
        round_id: roundId
      });
      await _application.default.updateMany({
        round_id: roundId,
        status: {
          $in: ['passed', 'failed']
        }
      ...
      const verifiedApps = await _application.default.find({
        round_id: roundId,
        status: 'verified'
      });
      const majorIds = [...new Set(verifiedApps.map(app => app.major_id.toString()))];
      const majors = await _major.default.find({
        _id: {
          $in: majorIds
        }
      });
      ...
      for (let aspOrder = 1; aspOrder <= maxAspiration; aspOrder++) {
        const appsAtLevel = verifiedApps.filter(app => app.aspiration_order === aspOrder && !admittedUsers.has(app.user_id.toString()));
        ...
        for (const [majorId, apps] of Object.entries(groupedByMajor)) {
          ...
          const eligible = apps.filter(app => app.score >= minimumScore).sort((a, b) => {
      ...
      for (const majorId of majorIds) {
        ...
        const result = await _admissionResult.default.create({
          round_id: roundId,
          major_id: majorId,
  ...
  async publishResult(roundId, adminId) {
    ...
    const results = await _admissionResult.default.find({
      round_id: roundId
    });
    if (results.length === 0) {
      throw new Error('Chưa có kết quả xét tuyển, vui lòng chạy xét tuyển trước');
    }
    await _admissionResult.default.updateMany({
      round_id: roundId
    }, {
      status: 'published',
    ...
    for (const app of applications) {
      ...
      if (app.status === 'passed') {
        await _notificationService.default.createAndPush(app.user_id, {
    ...
    return {
      ...
      results: await _admissionResult.default.find({
        round_id: roundId
      }).populate('major_id university_id')
    };
  }
  async getStatistics(roundId) {
    ...
    const totalApplications = await _application.default.countDocuments({
      round_id: roundId
    });
    const statusCounts = await _application.default.aggregate([{
      $match: {
        round_id: new _mongoose.default.Types.ObjectId(roundId)
      }
    }, {
      $group: {
        _id: '$status',
    ...
    const totalConfirmed = await _application.default.countDocuments({
      round_id: roundId,
      is_confirmed: true
    });
    return {
...
var _default = exports.default = new AdmissionService();