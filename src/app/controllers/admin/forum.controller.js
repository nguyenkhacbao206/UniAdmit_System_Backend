...
export const getOverview = async (req, res) => {
    try {
        const data = await forumAdminService.getOverview()
        return ok(res, data, 'Lấy tổng quan diễn đàn thành công')
...
export const getSettings = async (req, res) => {
    try {
        const data = await forumAdminService.getSettings()
        return ok(res, data, 'Lấy cấu hình thành công')
    } catch (err) { return fail(res, err, 500) }
}

export const updateSettings = async (req, res) => {
    try {
        const data = await forumAdminService.updateSettings(getActor(req), req.body)
        return ok(res, data, 'Cập nhật cấu hình thành công')
...
export const listLogs = async (req, res) => {
    try {
        const data = await forumAdminService.listLogs(req.query)
        return ok(res, data, 'Lấy lịch sử kiểm duyệt thành công')
    } catch (err) { return fail(res, err, 500) }
}
...
// Pending posts + moderation actions
